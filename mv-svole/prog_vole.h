...
#include "prog_base_cot.h"
#include "prog_base_svole.h"


using namespace std;
template <typename IO> class ProgVOLE {
public:
  IO *io;
  IO **ios;
  int party;
  ...
  ProgVOLE(int party, int threads, IO **ios, block seed,
    ...
    this->fixed_seed = seed;

    cot = new ProgBaseCot<IO>(party, io, fixed_seed, true);
    cot->cot_gen_pre();
  ...
  void setup() {
    ...
    if (party == ALICE) {
      ...
      svole0 = new ProgBasesvole<IO>(party, ios[0]);
  ...
  // the followings are our newly added functions

  void extend_with_seed(__uint128_t *buffer) {
    cot->cot_gen_with_seed(pre_ot, pre_ot->n);
    // memset(buffer, 0, n*sizeof(__uint128_t));
    if (party == ALICE)
      extend_send(buffer, mpfss, pre_ot, lpn, pre_yz);
    else
   ...
   void setup_with_seed(__uint128_t delta) {
    this->Delta = delta;
    setup_with_seed();
  }
  ...
  void setup_with_seed() {
    ...
    // generate tree_n*(depth-1) COTs
    int M_pre0 = pre_ot_ini0.n;
    cot->cot_gen_with_seed(&pre_ot_ini0, M_pre0);
    ...
    if (party == ALICE) {
      ...
      __uint128_t *mac = new __uint128_t[triple_n0];
      svole0 = new ProgBasesvole<IO>(party, ios[0]);
      svole0->triple_gen_recv_with_seed(mac, triple_n0, &fixed_seed);
    ...
    // generate tree_n*(depth-1) COTs
    int M_pre = pre_ot_ini.n;
    cot->cot_gen_with_seed(&pre_ot_ini, M_pre);
  ...
  void extend_with_seed(__uint128_t *data_yz, int num) {
    if (vole_triples == nullptr) {
      vole_triples = new __uint128_t[param.n];
      // memset(vole_triples, 0, n*sizeof(__uint128_t));
    }
