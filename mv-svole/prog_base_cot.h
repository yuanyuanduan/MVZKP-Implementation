...
class ProgBaseCot { public:
   int party;
	...
	ProgBaseCot(int party, IO *io, block seed, bool malicious = false) {
		this->party = party;
		this->io = io;
	...
	// our new added function
	void cot_gen_with_seed(block *ot_data, int64_t size) {
		if (this->party == ALICE) {
			iknp->send_cot(ot_data, size);
			io->flush();
			for(int64_t i = 0; i < size; ++i)
	...
	void cot_gen_with_seed(OTPre<IO> *pre_ot, int64_t size) {
		block *ot_data = new block[size];
		if (this->party == ALICE) {
			iknp->send_cot(ot_data, size);
			io->flush();
